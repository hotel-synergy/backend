...
const frontdeskRouter = require("./routes/v1/frontdesk");
const app = express();
...
mongoose.connect(process.env.DB_URI).then(() => {
  ...
  app.listen(process.env.SERVER_PORT, () => {
    console.log(
      `Application started and listening on port ${process.env.SERVER_PORT}`
    );
  });
});
...
app.use("/v1/frontdesk", frontdeskRouter);
...
app.use("*", async (req, res) => {
  ...
  if (acceptHeader && acceptHeader.includes("text/html")) {
    ...
    return res.status(404).json({
      error: true,
      message:
        "The requested resource was not found or is moved to the new address",
    });
  }
});
