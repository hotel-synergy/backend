...
const setupAdminAndCompany = async (req, res) => {
    ...
    */
    ...
    try {
        ...
        //hash the password 
        const hashedPassword = await bcrypt.hash(admin.password, parseInt(process.env.BCRYPT_SALT));
        ...
        const isEmailSent = await SendEmailVerificationLink(newEmailVerificationToken, admin.email, admin.name);
        ...
        const updatedUser = await User.findOneAndUpdate({ _id: newUser._id }, {
