...
const loginUser = async (req, res) => {
    ...
    const token = jwt.sign({ user: userInDB.fullName, role: userInDB.role }, process.env.JWT_SECRET.toString(), {
        algorithm: 'HS256'
    });
...
const resetUserPassword = async (req, res) => {
    ...
    const userInDB = await User.findOne({ email });
    if (!userInDB) {
        return res.status(404).json({ msg: 'That user was not found in the database.' });
    }

    //generate a new token for password reset.
    const linkToken = await jwt.sign({ _id: userInDB._id }, process.env.JWT_SECRET.toString(), {
        algorithm: 'HS256',
        expiresIn: '1d'
    });

    //set the token in db
    const updateUser = await User.findOneAndUpdate({ _id: userInDB._id }, {
